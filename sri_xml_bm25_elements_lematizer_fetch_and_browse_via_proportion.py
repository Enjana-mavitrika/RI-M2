...
for query_id in querys.keys():
    ...
    for doc_id in scores.keys():
        ...
        for element in dict(scores[doc_id]).keys():
            # intégrer le fetch and browse
            if element != ROOT :
                scores[doc_id][element] = scores[doc_id][ROOT] + scores[doc_id][element]
    ...
    with open('run_xml/FaresIbrahimaSolofo_{}_{}_bm25_elements_lematizer_k{}b{}_fetch_and_browse_via_proportion.txt'.format(NUM_RUN, NUM_FILE, k1, b), 'a') as run_file:
        run_file.write(run)
