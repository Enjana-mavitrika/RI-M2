            ...
            for sentence in text:
                ...
                for token in tokens:
                    ...
                    if token.isascii() and token not in stop_words and not token.isnumeric():
                        ...
                        # handle header
                        if is_inside_header:
                            elements_length[doc_id, h_path] += 1
                            if (doc_id, h_path, stem_term) in tf_e.keys():
                                tf_e[doc_id, h_path, stem_term] += 1
                            else :
                                tf_e[doc_id, h_path, stem_term] = 1
...
for query_id in querys.keys():
    ...
    with open('run_xml/FaresIbrahimaSolofo_{}_{}_bm25_articles_lematizer_k{}b{}_wilkinson94_pagerank.txt'.format(NUM_RUN, NUM_FILE, k1, b), 'a') as run_file:
        run_file.write(run)
