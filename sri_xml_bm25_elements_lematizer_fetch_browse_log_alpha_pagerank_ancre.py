...
for query_id in querys.keys():
            ...
            if term in df.keys():
                if (doc_id, term) in tf.keys():
                    ...
                    if ROOT in scores[doc_id].keys():
                        scores[doc_id][ROOT] += w + p_rank * ALPHA
                    else:
                        scores[doc_id][ROOT] = w + p_rank * ALPHA
    ...
    with open('run_xml/FaresIbrahimaSolofo_{}_{}_bm25_elements_lematizer_k{}b{}_fetch_browse_log_pagerank{}_ancre.txt'.format(NUM_RUN, NUM_FILE, k1, b, ALPHA), 'a') as run_file:
        run_file.write(run)
